...
Point estimates might not hit the exact population parameter we're looking for.
...
I'd be more surprised if 40 of them were left handed because that'd be the majority, and that's not what we usually observe in reality.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-Cw34875/manhattan.csv")
```
...
What does this mean?
It tells us that we're XX confident that our interval contains our true value.
...
boot_median
```
We are 95% sure that the true median is between 2162 and 2875.
...
- Population: Mice not in treatment
...
```{r practice_2_ci}
ci_mice_died <- get_ci(boot_prop, level = .90)
ci_mice_died
```
...
We are 90% confident that the proportion of mice that died while on the trial is between 0.552 and 0.651. This is a lot greater than the proportion of mice that died while not on treatment, meaning that the treatment was effective.
